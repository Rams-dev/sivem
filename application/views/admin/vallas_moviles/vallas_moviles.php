  ...
  $.get('vallas_moviles/obtenerImagenesVallasMovilesPorId/'+id, function(response){
    ...
    if(response == ''){
      ...
      let resp = JSON.parse(response);
         resp.map(res =>{
           $("#img1").attr('src',`<?= base_url()?>assets/images/vallas_moviles/${res.vista_corta}`);
           $("#img2").attr('src',`<?= base_url()?>assets/images/vallas_moviles/${res.vista_media}`);
           $("#img3").attr('src',`<?= base_url()?>assets/images/vallas_moviles/${res.vista_larga}`);
         })
    }
  })
}
